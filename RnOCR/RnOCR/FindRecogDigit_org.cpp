...
int CFindRecogDigit::Find_RecogGrayImg(BYTE* grayImg,int w,int h,char* pRes)
{
    ...
    if(h<200 || w<200) return -4;//recog_value;
	...
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\bin.bmp",binImg,w,h,1);
	...
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\cropgray.bmp",cropImg,w1,h1,8);
	...
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\bin.bmp",binImg,w1,h1,1);
	//delete[] zoomImg; zoomImg= NULL;

	//cropImg = binImg;
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\zoom.bmp",cropImg,w1,h1,1);

	BYTE* cropImg = CImageBase::CropImg(binImg,w1,h1,inRect);
	w1 = inRect.Width();
	h1 = inRect.Height();
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\crop.bmp",cropImg,w1,h1,1);
	for(i = 0; i < w1*h1; i++)	{
...
// 		for(j = 0; j < VIW; j ++)
// 		{
// 			x = polar_to_decart(i,j/2,w1/2,h1/2,y);
// 			if(x < 0 || x >= w1) continue;
// 			if(y < 0 || y >= h1) continue;
// 			pPolar[i*VIW+j] = cropImg[((int)y)*w1+(int)x];
// 		}
// 	}
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\polar.bmp",pPolar,VIW,VIH,1);
	...
	for(i=0;i<num;++i){
		...
		if(pt.y>avey[ord[1]]-gap/3 && avey[ord[1]]+gap/3 && pU->m_Rect.Height() > gap/2){
...
// 		}
		...
		//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\regular.bmp",pPolar,VIW,VIH,1);
		runProc.RemoveAllRunRt(rtAry);
		...
		for(i=0;i<3;++i){
			for(j=0;j<miniDotNum[i];++j){
				...
				vpos = path[ord[i]][xpos];//avey[ord[lineid]];
		...
		for(i=0;i<160;++i){
			...
			xpos = getDiffedPos(xpos0,rid,miniDotDiff[rid],miniDotNum[rid],rate);
			vpos = path[rid][xpos];//avey[ord[lineid]];
...
int CFindRecogDigit::getDiffedPos(int xpos,int lineid,double miniDotDiff[10],int miniDotNum,int rate)
{
	...
	double dx = ((xpos-x1)*dx1+(x2-xpos)*dx2)/(x2-x1);
	return (int)(xpos+dx); 
}
