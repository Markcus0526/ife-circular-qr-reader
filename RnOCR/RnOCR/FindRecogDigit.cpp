...
double g_miniDotangle[3][10]={	{35,	85,	157.5,	204.5,	275,	325},
								{65,	180,295},
								{45,	154,205,315}
};
...
int CFindRecogDigit::Find_RecogDib(BYTE* p24Dib,char* pResult)
{
	int w,h;
...
int CFindRecogDigit::Find_RecogGrayImg(BYTE* grayImg,int w,int h,char* pRes)
{
	int debug_flag = 1;
	CRunProc runProc;
	CRunRt* pU;
	...
	int i,j,ii,k,n;
	memset(pRes,0,sizeof(char)*40);
	// if width is larger than height of image, rotate 90
	if(h<200 || w<200) return recog_value;
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\1_gray.bmp",grayImg,w,h,8);
	// if image size is too large, reduce size.
	//BYTE* grayImg1 = CImageBase::MeanFilter(grayImg,w,h,30);
	//delete[] grayImg;
	//grayImg = grayImg1;
	for(i=0;i<5;++i)
		CImageBase::MeanFilter(grayImg,w,h);//,30);
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\2_meanfilter.bmp",grayImg,w,h,8);
	BYTE* binImg = Binarization_Windows(grayImg,w,h,12);
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\3_bin.bmp",binImg,w,h,1);

	int w1=w,h1=h;

	int num = runProc.MakeConnectComponentFromImg(binImg,w1,h1,rtAry);
	for(i=0;i<num;++i){
		pU = rtAry.GetAt(i);
		if(pU->m_Rect.left == 0 || pU->m_Rect.right == w1 || pU->m_Rect.top == 0 || pU->m_Rect.bottom == h1)
			pU->bUse = false;
		else if(pU->nPixelNum<10)
			pU->bUse = false;
		else if(pU->m_Rect.Width()<12 && pU->m_Rect.Height()<12 && pU->nPixelNum<80)
			pU->bUse = FALSE;
	}
	...
	CRect inRect = runProc.GetRealRect(rtAry);
	int w2 = inRect.Width();
	int h2 = inRect.Height();
	float frate = (float)w2/h2;
	if(frate<0.9 || frate>1.1){
		runProc.RemoveAllRunRt(rtAry);
		delete[] binImg; binImg= NULL;
		return recog_value;
	}
	runProc.GetImgFromRunRtAry(binImg,w1,h1,rtAry);
	runProc.RemoveAllRunRt(rtAry);
	...
	BYTE* cropImg = CImageBase::CropImg(binImg,w1,h1,inRect);
	delete[] binImg;binImg = NULL;
	w1 = inRect.Width();
	h1 = inRect.Height();
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\4_crop.bmp",cropImg,w1,h1,1);
	for(i = 0; i < w1*h1; i++)	{
		if(cropImg[i] == 1)		cropImg[i] = 0;
		else					cropImg[i] = 255;
	}
	...
	cropImg = zoomImg;
	for(i = 0; i < w1*h1; i++)	{
		if(cropImg[i] >128)		cropImg[i] = 0;
		else					cropImg[i] = 1;
	}
	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\5_zoom.bmp",cropImg,w1,h1,1);
	...
	for(i = 0; i < VIH; i++)
	{
		...
		{
			if(pPolar[i*VIW+j] == 0){
				if(pPolar[i*VIW+j-1] == 1 && pPolar[i*VIW+j+1] == 1)
					pPolar[i*VIW+j] = 1;
			}
		}
	}

	//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\6_polar.bmp",pPolar,VIW,VIH,1);
	memcpy(pPolar1,pPolar,VIW*VIH);

	num = runProc.MakeConnectComponentFromImg(pPolar,VIW,VIH,rtAry);
	for(i=0;i<num;++i){
		pU = rtAry.GetAt(i);
		if(pU->m_Rect.top < 140)
			pU->bUse = FALSE;
	}
	num = runProc.RemoveNoneUseRunRt(rtAry);
	runProc.GetImgFromRunRtAry(pPolar,VIW,VIH,rtAry);
	runProc.RemoveAllRunRt(rtAry);

	
	float (*path)[2000];
	float (*path1)[2000];
	path = new float[3][2000];
	path1 = new float[3][2000];
	int avey[3],avey1[3],ord[3],gap;
	for(i=0;i<3;++i)
	{
		ord[i] = i;
		avey1[i] = CircleViterbi_SAMZHU(pPolar,VIW,VIH,path1[i]);
		RemoveLine(pPolar,VIW,VIH,path1[i]);
	}
	...
	for(i=0;i<3;++i){
		avey[i] = avey1[ord[i]];
		memcpy(path[i],path1[ord[i]],sizeof(float)*2000);
	}
	delete[] path1;
	...
	int bit8s[4][8];
	char charbit[161];
	char charcheckbit[33];
	memset(charbit,0,161);
	memset(charcheckbit,0,33);

	char testcodebit[]="0100010111110001100100110111000110111001111000100000111101110100111000001110100001101010000101001010010110111001001001100110001000110101101001101110001011111010";
	char testcheck[] = "10011000011100100101000110011110";
	...
	int    miniDotNum[3]={6,3,4};
	double miniDotDiff[3][10];
	memset(miniDotDiff,0,sizeof(double)*30);
	num = runProc.MakeConnectComponentFromImg(pPolar1,VIW,VIH,rtAry);

	memset(axisid,0,sizeof(int)*3);
	for(i=0;i<num;++i){
		pU = rtAry.GetAt(i);
		pt = pU->m_Rect.CenterPoint();
		if(pt.y>avey[1]-gap/3 && avey[1]+gap/3 && pU->m_Rect.Height() > gap/2){
	...
	if(axisid[0]>0 && axisid[1]>0 && axisid[2]>0){
		...
		//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\removeaxis.bmp",pPolar1,VIW,VIH,1);
		...
		//if(debug_flag) CImageBase::SaveImgToFile("d:\\temp\\7_regular.bmp",pPolar,VIW,VIH,1);
		runProc.RemoveAllRunRt(rtAry);
		num = runProc.MakeConnectComponentFromImg(pPolar,VIW,VIH,rtAry);
		int vpos,xpos,lineid;

		num = rtAry.GetSize();
		for(i=0;i<3;++i){
			for(j=0;j<miniDotNum[i];++j){
				xpos = (int)(g_miniDotangle[i][j]*rate+0.5f);
				vpos = path[i][xpos];//avey[ord[lineid]];
				miniDotDiff[i][j] = getMiniDotDiff(xpos,vpos,rtAry);
			}
		}
		runProc.RemoveAllRunRt(rtAry);
		...
		for(i=0;i<160;++i){
			...
			xpos0 = (int)(g_bitangle[i]*rate+0.5f);
			//xpos = getDiffedPos(xpos0,ord[lineid],miniDotDiff[lineid],miniDotNum[lineid],rate);
			xpos = getDiffedPos(xpos0,lineid,miniDotDiff[lineid],miniDotNum[lineid],rate);
			vpos = path[lineid][xpos];//avey[ord[lineid]];
			codebit[i] = GetBitFromImage(pPolar,VIW,VIH,xpos,vpos);//g_bitangle[i]*rate,vpos);
			charbit[i] = codebit[i]+0x30;
		}
		for(i=0;i<32;++i){
			if(i>=0 && i<16) 
				lineid = 0;
			...
			xpos0 = (int)(g_checkbitangle[i]*rate+0.5f);
			//xpos = getDiffedPos(xpos0,ord[lineid],miniDotDiff[lineid],miniDotNum[lineid],rate);
			xpos = getDiffedPos(xpos0,lineid,miniDotDiff[lineid],miniDotNum[lineid],rate);
			vpos = path[lineid][xpos];//avey[ord[lineid]];
			checkbit[i] = GetBitFromImage(pPolar,VIW,VIH,xpos,vpos);//g_checkbitangle[i]*rate,vpos);
			charcheckbit[i] = checkbit[i]+0x30;
		}
		{
			for(i=0;i<160;++i){
				if(codebit[i] != testcodebit[i]-0x30){
					int a= 1;
				}
			}
			for(i=0;i<32;++i){
				if(checkbit[i] != testcheck[i]-0x30){
					int a= 1;
				}
			}
		}
...
int CFindRecogDigit::GetBitFromImage(BYTE* pPolar,int w, int h ,int xpos,int vpos)//double angle,int vpos)
{
	int gap = 5;
	int i,j,j1,k,fg = 1,count=0,sum1;
	//for(j = 0; j < w; j ++)
	j = xpos;//(int)(angle+0.5f);
	//j = (int)(angle-1);
...
int CFindRecogDigit::getDiffedPos(int xpos,int lineid,double miniDotDiff[10],int miniDotNum,int rate)
{
	int i,x1,x2;
	double dx1,dx2;
	if(xpos<g_miniDotangle[lineid][0]*rate){
		x1 = 0;
		x2 = g_miniDotangle[lineid][0]*rate;
		dx1 = 0;
		dx2 = miniDotDiff[0];
	}
	else if(xpos>g_miniDotangle[lineid][miniDotNum-1]*rate){
		x1 = g_miniDotangle[lineid][miniDotNum-1]*rate;
		x2 = 360*rate;
		dx1 = miniDotDiff[miniDotNum-1];
		dx2 = 0;
	}
	else{
		for(i=0;i<miniDotNum-1;++i){
			if(xpos>g_miniDotangle[lineid][i]*rate && xpos<g_miniDotangle[lineid][i+1]*rate){
				x1 = g_miniDotangle[lineid][i]*rate;
				x2 = g_miniDotangle[lineid][i+1]*rate;
				dx1 = miniDotDiff[i];
				dx2 = miniDotDiff[i+1];
				break;
			}
	...
	//if(dx1+dx2<0.0001) return xpos;
	double dx = ((double)(xpos-x1)*dx2+(x2-xpos)*dx1)/(x2-x1);
	return (int)(xpos+dx); 
}

double CFindRecogDigit::getMiniDotDiff(int xpos,int vpos,CRunRtAry& rtAry)
{
	double miniDotDiff = 0;
	int i,num,minDis=100000,minId=0,Dis;
	CRect r;
	int cx,cy;
	num = rtAry.GetSize();
	if(num == 0) return 0;
	for(i=0;i<num;++i){
		...
		cx = (r.left+r.right)/2;
		cy = (r.top+r.bottom)/2;
		Dis = (cx-xpos)*(cx-xpos)+(cy-vpos)*(cy-vpos);
		if(Dis<minDis){
			minDis = Dis;
			minId = i;
		}
	}
	r = rtAry[minId]->m_Rect;
	cx = (r.left+r.right)/2;
	cy = (r.top+r.bottom)/2;
	miniDotDiff = (double)(cx-xpos);//(xpos-cx);
	return miniDotDiff;
}
int CFindRecogDigit::CircleViterbi_SAMZHU(BYTE* pPolar,int w,int h,float* hpath)
{
	...
	for(i = h/5; i < h; i ++)
	{
		for(k = -VITERBISEGHEIGHT; k <= VITERBISEGHEIGHT; k ++)
		{
			...
			{
				memcpy(hpath1,hpath,sizeof(float)*2000);
				t1 = i;t2 = k;
				...
				hpath[0] = t1;
				if(abs(t1 - i) > 2) {memcpy(hpath,hpath1,sizeof(float)*2000);continue;}
