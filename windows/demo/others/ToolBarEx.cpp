	...
	, m_uTimerEvent(0)
{
	...
	if( bOldSys ) {
		...
		lstrcpy(logfont.lfFaceName,L"MS Sans Serif");
	}
	logfont.lfHeight = 6 ;
...
void CToolBarEx::MyToolBarSettingInfo(UINT Bar_ID,UINT Bar_Style)
{
	UINT nID,nStyle;
	int nIndex,iImage;
	nIndex=CommandToIndex(Bar_ID);
	GetButtonInfo(nIndex,nID,nStyle,iImage);
	SetButtonInfo(nIndex,nID,Bar_Style,iImage);
}
void CToolBarEx::GetMyToolBarRect(UINT Bar_ID,CRect &rect)
{
	int nIndex;
	nIndex=CommandToIndex(Bar_ID);
	GetItemRect(nIndex,rect);
}
int CToolBarEx::GetToolBarHigh(UINT Bar_ID)
{
	int nIndex=CommandToIndex( Bar_ID);
	CRect rect;
	GetItemRect(nIndex, rect);
