...
// if CCITT_FAX3_2D Tiff,return 8;
DWORD CxImage::GetCodecOptionEx(DWORD imagetype)
{
	if (imagetype<CMAX_IMAGE_FORMATS){
		if (imagetype==0){
			imagetype = GetType();
		}
		if (imagetype == 6 && m_bFax3_2D)
			return 8;//COMPRESSION_CCITTFAX3_2D
		return info.dwCodecOpt[imagetype];
	}
	return 0;
}
...
bool CxImage::SetCodecOptionEx(DWORD opt, DWORD imagetype)
{
	if (imagetype<CMAX_IMAGE_FORMATS){
		if (imagetype==0){
			imagetype = GetType();
		}
		if (imagetype == 6 && opt == 8){//Tiff ÀÌ¸י COMPRESSION_CCITTFAX3_2D
			m_bFax3_2D = TRUE;
			opt = 3;//COMPRESSION_CCITTFAX3_1D
		}
		info.dwCodecOpt[imagetype] = opt;
		return true;
	}
...
////////////////////////////////////////////////////////////////////////////////
char* CxImage::GetComment()
{
	return (char*)&info.szComment[0];
}
void CxImage::SetComment(const char* szComment)
{
	if (szComment) strncpy(info.szComment,szComment,255); 
}