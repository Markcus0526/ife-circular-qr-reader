...
/** \addtogroup Attributes */ //@{
	...
	DWORD   GetCodecOptionEx(DWORD imagetype = 0);
	bool	SetCodecOption(DWORD opt, DWORD imagetype = 0);
	bool	SetCodecOptionEx(DWORD opt, DWORD imagetype = 0);
...
/** \addtogroup Painting */ //@{
	...
	HANDLE	CopyToHandle();
	void	MyCreateFromHANDLE(HANDLE hMem);
...
/** \addtogroup Decode */ //@{
	...
	bool MyIsValidImageFile(const TCHAR * filename,DWORD imagetype);
	void MyModifyBinaryImageRGBQUAD();
//@}
...
/** \addtogroup BasicTransformations */ //@{
	...
	bool MyShiftImage(int w,int h,int Diffx,int Diffy);
//@}
...
/** \addtogroup DSP */ //@{
	...
	bool  Despeckle();
	BYTE* MyRotateGrayBits(BYTE *pBits, int &width, int &height,double fAng,BYTE GrayBackGround,int bKeepSize =TRUE);
//@}
