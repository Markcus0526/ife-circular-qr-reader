...
bool CxImage::Encode(CxFile *hFile, DWORD imagetype)
{
	...
	if (imagetype==CXIMAGE_FORMAT_GIF){
		CxImageGIF newima;
		newima.Ghost(this);
		newima.SetComment(info.szComment);//ksd 2007.7.17
		if (newima.Encode(hFile)){
			return true;
		} else {
			strcpy(info.szLastError,newima.GetLastError());
...
#endif
#if CXIMAGE_SUPPORT_SMP
	if (imagetype==CXIMAGE_FORMAT_SMP){
		//CxImageSMP newima;
		//newima.Ghost(this);
		//if (newima.Encode(hFile)){
		//	return true;
		...
		wchar_t* pw;
		DWORD type;
		if(GetBpp()==1)		type = CXIMAGE_FORMAT_TIF;
		else				type = CXIMAGE_FORMAT_JPG;
		memset(hdr,0,1024);
		sprintf(hdr,"SM 2015.08.15");
		hdr[13] = (char)type;
		p = hdr+14;
		sprintf(p,"unicode");
		p = p+7;
		pw = (wchar_t*)p;
		wcscpy(pw,(wchar_t*)info.szFileName);
		if(GetBpp()==1){
			CxImage xtif;
			xtif.Copy(*this);
			xtif.SetCodecOption(COMPRESSION_CCITT_T6,type);
			long size=0;
			BYTE* buffer=0;
			xtif.Encode(buffer,size,type);
			if(buffer != NULL && size>0){
				hFile->Write(&hdr,1024,1);
				hFile->Write(buffer,size,1);
				delete buffer;
				return true;
			}
		}
			...
			CxImage xjpg;
			xjpg.Copy(*this);
			xjpg.SetJpegQuality(50);
			long size=0;
			BYTE* buffer=0;
			xjpg.Encode(buffer,size,type);
			if(buffer != NULL && size>0){
				hFile->Write(&hdr,1024,1);
				hFile->Write(buffer,size,1);
				delete buffer;
...
//bool CxImage::Load(const char * filename, DWORD imagetype)
{
	...
	fclose(hFile);*/
	wcscpy((wchar_t*)info.szFileName,filename);
...
bool CxImage::Decode(CxFile *hFile, DWORD imagetype)
{
	...
	if (imagetype==CXIMAGE_FORMAT_GIF){
		...
		if (newima.Decode(hFile)){
			Transfer(newima);
			newima.GetComment(info.szComment);//ksd 2007.7.17
			return true;
		} else {
			info.nNumFrames = newima.info.nNumFrames;
			strcpy(info.szLastError,newima.GetLastError());
	...
	if (imagetype==CXIMAGE_FORMAT_SMP){
		...
		int fLen = hFile->Size();
		hFile->Read(hdr,1024,1);
		fLen -= 1024; 
		BYTE* buffer = new BYTE[fLen];
		hFile->Read(buffer,fLen,1);
		DWORD type = (DWORD)hdr[13];
		bool bOk = Decode(buffer, (DWORD)fLen, type);
		delete[] buffer;
		MyModifyBinaryImageRGBQUAD();
		if (bOk) return true;
		else {
			strcpy(info.szLastError,GetLastError());
			return false;
		}
	}
#endif
...
void CxImage::MyModifyBinaryImageRGBQUAD()
{
	if(!pDib) return;
	if(GetBpp() != 1) return;
	...
	BYTE* pBits = GetBits();
	for(int i=0;i<head.biSizeImage;++i) pBits[i] = ~pBits[i];
}
////////////////////////////////////////////////////////////////////////////////
