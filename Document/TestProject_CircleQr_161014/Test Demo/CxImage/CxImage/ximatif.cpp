...
bool CxImageTIF::Decode(CxFile * hFile)
{
  ...
  try{
	...
	Fax3BaseState* sp = Fax3State(m_tif);
	if (sp && sp->groupoptions == 1)
		m_bFax3_2D = TRUE;
...
//	TIFFSetField(m_tif, TIFFTAG_YPOSITION, (float)info.yOffset);
	...
	if (m_bFax3_2D){
		TIFFSetField(m_tif,TIFFTAG_GROUP3OPTIONS,GROUP3OPT_2DENCODING);
		
		TIFFSetField(m_tif,TIFFTAG_FAXMODE, FAXMODE_CLASSIC);//CLASSIC방식인 
		
		//CLASSF방식인 경우
		//TIFFSetField(m_tif,TIFFTAG_FAXMODE, FAXMODE_CLASSF);		
	}
