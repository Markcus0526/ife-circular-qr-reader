...
////////////////////////////////////////////////////////////////////////////////
bool CxImage::MyShiftImage(int w,int h,int Diffx,int Diffy)
{
	if (!pDib) return false;

	CxImage imgDest;
	imgDest.CopyInfo(*this);
	imgDest.Create(w,h,GetBpp(),GetType());
	imgDest.SetPalette(GetPalette());

	BYTE *iDst=imgDest.GetBits();
	memset(iDst,255,imgDest.head.biSizeImage);
	int i,j,i1,j1,ii;
	int w2 = GetWidth();
	int h2 = GetHeight();
	...
	for(i=0;i<h;++i)for(j=0;j<w;++j){
		...
		j1 = j-Diffx;
		ii = h-1-i;
		if(i1<0 || i1 >= h2) continue;
		if(j1<0 || j1 >= w2) continue;
		if (head.biClrUsed==0){ //RGB
			imgDest.SetPixelColor(j,ii,GetPixelColor(j1,i1));
		}
		else { //PALETTE
			imgDest.SetPixelIndex(j,ii,GetPixelIndex(j1,i1));
		}
	}
...
bool CxImage::RotateLeft(CxImage* iDst)
{
	...
	if (head.biBitCount == 1) {
		...
		for (y = 0; y < head.biHeight; y++) {
			...
			div_r = div(long(y + dlineup), long(8));
...
//
BYTE* CxImage::MyRotateGrayBits(BYTE *pBits, int &width, int &height,double fAng,BYTE GrayBackGround,int bKeepSize/*=TRUE*/)
{
	BYTE* pNewBits;
	int w = width;
	int h = height;
	...
	if (bKeepSize){
		...
		for (int i=0; i<4; i++) {
				...
				newpY[i] = (pX[i]*sin_angle + pY[i]*cos_angle);
		}
	}
	...
	pNewBits = new BYTE[newEffWidth*height];
	memset(pNewBits,0,newEffWidth*height);
	...
	for (desty=0; desty<height; desty++) {
		...
		for (destx=0; destx<width; destx++) {
			...
			if (nearx>0 && nearx<=w-1 && neary>0 && neary<=h-1)
			{
				...
				pNewBits[desty*newEffWidth+destx] = max(0,min(255,(a*a1+b*b1+c*c1+d*d1))); 
			}
			else				
