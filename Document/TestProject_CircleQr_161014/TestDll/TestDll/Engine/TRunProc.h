...
public:
	...
	void ReAdjustLeft(int NewLeft)
	{//
		if(NewLeft < m_Rect.left || NewLeft >= m_Rect.right) return;
		int i,x1,x2,xx1,y1,y2,Ct;
		DWORD* NewData = new DWORD[nRunNum];
		Ct=0;
		for(i=0;i<nRunNum;i+=2){
			x1 = (int)LOWORD(pRunData[i]);
			x2 = (int)LOWORD(pRunData[i+1]);
			y1 = (int)HIWORD(pRunData[i]);
			y2 = (int)HIWORD(pRunData[i+1]);
			if(NewLeft >= x2) continue;
			xx1 = max(x1,NewLeft);
			NewData[Ct] = MAKELONG((WORD)xx1,(WORD)y1);
			NewData[Ct+1] = MAKELONG((WORD)x2,(WORD)y2);
			Ct = Ct+2;
		}
		delete pRunData;pRunData=NULL;
		pRunData = NewData;
	...
	void ReAdjustRight(int NewRight)
	{//
		if(NewRight < m_Rect.left || NewRight >= m_Rect.right) return;
		int i,x1,x2,xx2,y1,y2,Ct;
		DWORD* NewData = new DWORD[nRunNum];
		Ct=0;
		for(i=0;i<nRunNum;i+=2){
			x1 = (int)LOWORD(pRunData[i]);
			...
			y1 = (int)HIWORD(pRunData[i]);
			y2 = (int)HIWORD(pRunData[i+1]);
			if(NewRight <= x1) continue;
			xx2 = min(x2,NewRight);
			NewData[Ct] = MAKELONG((WORD)x1,(WORD)y1);
			NewData[Ct+1] = MAKELONG((WORD)xx2,(WORD)y2);
			Ct = Ct+2;
		}
		delete pRunData;pRunData=NULL;
		pRunData = NewData;
	...
	void ReAdjustTop(int NewTop)
	{//
		if(NewTop < m_Rect.top || NewTop >= m_Rect.bottom) return;
		int i,x1,x2,y1,y2,Ct;
		DWORD* NewData = new DWORD[nRunNum];
		Ct = 0;
		for(i=0;i<nRunNum;i+=2){
			x1 = (int)LOWORD(pRunData[i]);
	...
	void ReAdjustBottom(int NewBottom)
	{
		if(NewBottom < m_Rect.top || NewBottom >= m_Rect.bottom) return;
		
		int i,x1,x2,y1,y2,Ct;
		DWORD* NewData = new DWORD[nRunNum];
		Ct=0;
		for(i=0;i<nRunNum;i=i+2){
			x1 = (int)LOWORD(pRunData[i]);
			x2 = (int)LOWORD(pRunData[i+1]);
			y1 = (int)HIWORD(pRunData[i]);
			y2 = (int)HIWORD(pRunData[i+1]);
			if(y1 >= NewBottom) continue;
			NewData[Ct] = MAKELONG((WORD)x1,(WORD)y1);
			NewData[Ct+1] = MAKELONG((WORD)x2,(WORD)y2);
			Ct = Ct+2;
		}
	...
	void ReSetRectSg()
	{
		int i,x1,x2,aSS,y1,y2;
		int l=10000,r=0,t=10000,b=0;
		aSS=0;
		for(i=0;i<nRunNum;i=i+2){
			x1 = (int)LOWORD(pRunData[i]);
			x2 = (int)LOWORD(pRunData[i+1]);
			...
			t = min(t,y1); b = max(b,y1);
			aSS += x2-x1;
		}
		if(nRunNum>1)
			m_Rect = CRect(l,t,r,b+1);
		else
			m_Rect = CRect(0,0,0,0);
		nPixelNum = aSS;
	};
